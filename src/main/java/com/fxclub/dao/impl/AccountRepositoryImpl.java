...
public class AccountRepositoryImpl implements AccountRepository {
    ...
    public Optional<Account> findById(Integer id) {
        ...
        try {
            account = jdbcTemplate.queryForObject("SELECT id, balance FROM account WHERE id = ?", new RowMapper<Account>() {
                @Override
    ...
    public Optional<Account> findByIdForManipulation(Integer id) {
        Account account = null;
        try {
            account = jdbcTemplate.queryForObject("SELECT id, balance FROM account WHERE id = ? FOR UPDATE", new RowMapper<Account>() {
                @Override
                public Account mapRow(ResultSet rs, int rowNum) throws SQLException {
                    Account rsAccount = new Account();
                    rsAccount.setId(rs.getInt("id"));
